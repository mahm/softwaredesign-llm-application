...
4. HTML本文が取得できない場合や数百字程度のスタブしか返らない場合(2023年以前の論文に多い)は、`curl -sL https://ar5iv.labs.arxiv.org/html/{id}` で全文を取得する
5. ar5ivでも本文が取得できない場合(PDF限定の論文)はユーザーに伝え、対象外とする
6. HTMLタグからセクション構造(h2, h3)を読み取る
7. 数式はTeX表記のまま抽出し、図表の参照はテキストで要約する
8. タイトル、著者、アブストラクト、各セクションの内容を抽出する
