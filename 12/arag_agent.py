...
class ResearchState(BaseModel):
    ...
    method: Literal["A", "B", "C"] = Field(default="A", description="選択された方法")
...
class AdaptiveRagAgent:
    ...
    def _run_method_classifier(self, state: ResearchState) -> dict[str, Any]:
        """メソッド分類器を実行する"""
    ...
    def _run_non_retrieval_qa(self, state: ResearchState) -> dict[str, Any]:
        """非検索型QAを実行する"""
        prompt = ChatPromptTemplate.from_messages(
    ...
    def _run_single_step_approach(self, state: ResearchState) -> dict[str, Any]:
        """単一ステップアプローチを実行する"""
    ...
    def _run_multi_step_approach(self, state: ResearchState) -> dict[str, Any]:
        """複数ステップアプローチを実行する"""
        inputs = {"messages": [("user", state.task)]}
    ...
    def stream(self, task: str) -> str:
        ...
        initial_state = ResearchState(task=task)
        final_output = ""
